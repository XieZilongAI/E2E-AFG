...
PSEUDO_ANSWER_PREFIX = "pseudo_answer"
...
def get_psean(
    ...
    has_answer = False
    if criteria == "strinc":
        if example['pseudo_answer'][criteria]:
            has_answer = True
        # break
    elif example['pseudo_answer'][criteria] >= threshold:
        has_answer = True
    sentences.append(example["pseudo_answer"]["text"])
    sent_text = " ".join(sentences)
    if len(sent_text) > 1000:
        sent_text = sent_text[:1000]
    return sent_text, has_answer
...
def get_example_io(
        ...
        pseudo_answer_prefix: str = "pseudo_answer",
) -> tuple[str, str]:
    """Get input and output texts for the given example."""
    input_text_list, classification_labels, output_text_list = [], [], []
    for inp in input_list:
        if inp == "question":
            ...
            input_text_list.append(get_context(example, n_contexts, context_prefix=context_prefix, add_prefix=True))
        elif inp == "filtered":
            sent = get_sent(
                example=example,
            ...
            psean, has_answer = get_psean(example, criteria=filter_criteria)
            if not has_answer:
                classification_labels.append(0)
            else:
                classification_labels.append(1)
            psean = prefix_format.format(pseudo_answer_prefix, psean)

            input_text_list.append(psean)
            if not sent.strip():
                classification_labels.append(0)
                sent = get_context(example, n_contexts, context_prefix=context_prefix)
            else:
                classification_labels.append(1)
    ...
    input_text = SEP_TOKEN.join(input_text_list)
    ...
    output_text += SEP_TOKEN + "classification_labels: " + str(classification_labels)
...
def get_ict_io(
    ...
    for example in in_context_examples:
        itext, otext = get_example_io(
            ...
            pseudo_answer_prefix=pseudo_answer_prefix,
        )
        ict_io_list.append(SEP_TOKEN.join([itext, otext]))
    ...
    if no_prefix:
        ...
        for ii in input_list:
            if (ii == "filtered") or (ii == "passage"):
                ...
                input_text_list.append(pseudo_answer_prefix)
            else:
    ...
    full_input_text = SEP_TOKEN.join([prefix] + ict_io_list + [input_text])
